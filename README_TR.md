...
Sidebar'daki **Tüneller** bölümünden Local (`-L`), Remote (`-R`) ve Dynamic (`-D`) forward tanımları oluşturabilirsin. Her tünel bir hedef Host alias'ına bağlanır ve tek tel başlatılıp durdurulabilir; "Otomatik Bağlan" işaretliyse tünel ilgili bağlantı açıldığında kendiliğinden kurulur. Varsayılan yerel bind adresi `127.0.0.1`'dir; `0.0.0.0` veya `::` gibi dışa açık bir adres seçilirse arayüz güvenlik uyarısı gösterir.
